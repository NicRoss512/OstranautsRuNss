#!/usr/bin/env python3
"""
translate_selective.py v2 — только строки, без путей и номеров внутри docx

Теперь в docx только конкретные строки:
- никаких File/Key/ID внутри документа — Яндекс их переводил в "реклама/ads.json"
- просто по боксам (параграфам/ячейкам) считаем строки, порядок хранится в mapping.json
...
def export_docx(args):
    ...
    print(f"Словарь после: {len(dict_map)} (пустых [is]->'': {sum(1 for v in dict_map.values() if not v)}) — теперь знаешь с чем имеешь дело: {sorted(found_ph)}")
